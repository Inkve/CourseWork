...
void triangle::move(int _x, int _y) {
	...
	if (_x == 0 || _y == 0) {
		if (_x == 0 && _y != 0) {
			throw(Errors("Нулевая координата для перемещения!", "_x", _x, NULL));
		};
		if (_x != 0 && _y == 0) {
			throw(Errors("Нулевая координата для перемещения!", "_y", _y, NULL));
		};
		throw(Errors("Нулевые координаты для перемещения!", "_x и _y", _x, _y));
	};
